...
public class FlightAPIActivity extends Activity implements APICallback, AdapterView.OnItemClickListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etStart = forceCast(findViewById(R.id.etStart));
        etEnd = forceCast(findViewById(R.id.etEnd));
        etNo = forceCast(findViewById(R.id.etNo));
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
        ...
        if (currentIsQueryByNo) {
            ...
            if (Math.random() < 0.5) {
                //使用机场三字码
                start = forceCast(item.get("fromAirportCode"));
                end = forceCast(item.get("toAirportCode"));
            } else {
                //使用城市名称
                start = forceCast(item.get("fromCityName"));
                end = forceCast(item.get("toCityName"));
            }
        ...
        etStart.setText("");
        etEnd.setText("");
        etNo.setText("" + com.mob.tools.utils.R.<String>forceCast(item.get("flightNo")));
        ((Flight) MobAPI.getAPI(Flight.NAME)).queryByNo(com.mob.tools.utils.R.<String>forceCast(item.get("flightNo")), FlightAPIActivity.this);
    }
    ...
    private void showResult(ArrayList<Map<String, Object>> result) {
        ...
        if (adapter == null) {
            adapter = new SimpleAdapter(this, listResult, R.layout.view_flight_info_list_item,
                    new String[]{"airLines", "flightNo", "flightRate", "flightTime", "from", "fromAirportCode", "fromCityCode", "fromCityName", "fromTerminal", "planTime", "planArriveTime", "to", "toAirportCode", "toCityCode", "toCityName", "toTerminal", "week"},
                    new int[]{R.id.tvAirLines, R.id.tvFlightNo, R.id.tvFlightRate, R.id.tvFlightTime, R.id.tvFrom, R.id.tvFromAirportCode, R.id.tvFromCityCode, R.id.tvFromCityName, R.id.tvFromTerminal, R.id.tvPlanTime, R.id.tvPlanArriveTime, R.id.tvTo, R.id.tvToAirportCode, R.id.tvToCityCode, R.id.tvToCityName, R.id.tvToTerminal, R.id.tvWeek});
